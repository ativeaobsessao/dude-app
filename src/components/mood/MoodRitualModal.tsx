...
export const MoodRitualModal = () => {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-base/80 backdrop-blur-md">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 10 }}
          transition={{ duration: 0.3, ease: 'easeOut' }}
          className="relative w-full max-w-lg overflow-hidden rounded-3xl bg-surface-2/95 border border-border-custom p-8 md:p-10 shadow-2xl text-center"
        >
          ...
          <div className="flex flex-col items-center gap-6 relative z-10">
            <div className="w-12 h-12 rounded-2xl bg-green/10 flex items-center justify-center text-green shadow-[0_0_20px_rgba(110,231,168,0.15)]">
              <Heart size={20} className="fill-green/10" />
            </div>

            <div className="space-y-2">
              <span className="text-[10px] font-mono font-bold uppercase tracking-[0.25em] text-green block">
                MOOD RITUAL
              </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 w-full mt-4">
              {MOOD_LIST.map((m) => (
                <button
                  key={m.key}
                  onClick={() => handleSelectMood(m.key)}
                  className="group relative flex flex-col items-center gap-2.5 p-4 rounded-2xl bg-surface-1/40 hover:bg-surface-1/90 border border-border-custom/50 hover:border-border-custom transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 active:scale-95 cursor-pointer"
                  style={{
                    '--hover-glow': m.color
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-1.5 mt-1">
                    ...
                    />
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-dim group-hover:text-text transition-colors text-center leading-tight">
                      {m.label}
                    </span>
                  </div>
                </button>
            ...
            <button
              onClick={handleSkip}
              className="mt-4 text-[10px] font-bold uppercase tracking-widest text-text-dim/40 hover:text-text transition-colors cursor-pointer"
            >
